...
export function getTxHistory(): TxHistoryEntry[] {
  ...
  try {
    ...
    return parsed
      .filter(
        (item) => item?.digest && item?.action && item?.createdAt && item?.area,
      )
      .map((item) => ({
...
export function recordTx(payload: Omit<TxHistoryEntry, "createdAt">): void {
  ...
  const deduped = current.filter(
    (item) =>
      !(item.digest === nextEntry.digest && item.action === nextEntry.action),
  );
