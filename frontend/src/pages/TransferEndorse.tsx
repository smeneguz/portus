...
export default function TransferEndorse() {
  ...
  const readCreatedChainId = (
    ...
    const created = changes.find(
      (change) =>
        change.type === "created" &&
        change.objectType?.includes("endorsement::EndorsementChain"),
    );
  ...
  const handleCreateChain = async () => {
    ...
    try {
      ...
      if (createdChainId) {
        ...
        setEblId((current) => current || newChainBlId);
      }
      ...
      if (result.digest) {
        ...
        recordTx({
          ...
          area: "transfer",
          referenceId: createdChainId || newChainBlId || undefined,
          referenceLabel: createdChainId ? "Chain ID" : "eBL ID",
          details: `Chain initialized for eBL ${newChainBlId.slice(0, 10)}... with initial holder ${initialHolder.slice(0, 10)}...`,
        });
      }
      setMessage({
        kind: "ok",
        text: createdChainId
          ? `Endorsement chain created: ${createdChainId}`
          : "Endorsement chain created successfully. If chain ID is still empty, click Refresh in History and paste the chain object ID.",
      });
    } catch (err) {
      console.error("Create chain failed:", err);
      setMessage({
        kind: "error",
        text: "Failed to create chain. Check IDs and wallet role.",
      });
    }
  };
  ...
  const handleTransfer = async () => {
    ...
    try {
      ...
      setMessage({
        kind: "ok",
        text: "Endorsement and transfer submitted on-chain.",
      });
    } catch (err) {
      console.error("Transfer failed:", err);
      setMessage({
        kind: "error",
        text: "Transfer failed. Ensure you are the current holder of the eBL.",
      });
    }
  };
    ...
    return (
      <div className="surface p-10 text-center">
        ...
        <p className="section-subtitle mt-2">
          Connect your IOTA wallet to endorse and transfer title.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <section className="surface p-5 md:p-6">
        ...
        <p className="section-subtitle mt-1">
          Use one chain per eBL and reuse that chain for each transfer.
        </p>
        <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3">
          <div className="rounded-xl border border-[#d7e2ef] bg-white p-3">
            <p className="text-xs font-semibold uppercase tracking-wide text-[#3d5e81]">
              1. Create chain
            </p>
            <p className="mt-1 text-sm text-[#4f657d]">
              Initialize once with eBL ID and initial holder address.
            </p>
          </div>
          <div className="rounded-xl border border-[#d7e2ef] bg-white p-3">
            <p className="text-xs font-semibold uppercase tracking-wide text-[#3d5e81]">
            ...
            <p className="mt-1 text-sm text-[#4f657d]">
              Submit recipient address + endorsement type from current holder
              wallet.
            </p>
          </div>
          <div className="rounded-xl border border-[#d7e2ef] bg-white p-3">
            ...
            <p className="mt-1 text-sm text-[#4f657d]">
              Check history timeline to see tx digests and on-chain endorsement
              events.
            </p>
          </div>
        </div>
      ...
      <section className="surface p-5 md:p-6">
        ...
        <p className="section-subtitle mt-1">
          Run once for each eBL before the first transfer in the chain of title.
        </p>
        ...
        <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            ...
            <input
              ...
              placeholder="0x..."
              value={newChainBlId}
              onChange={(e) => setNewChainBlId(e.target.value)}
            />
          </div>
          <div>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          <button
            ...
            disabled={chainPending || !newChainBlId || !initialHolder}
            className="btn-main"
          >
            {chainPending ? "Creating chain..." : "Create Chain"}
          </button>
          <p className="text-xs text-[#5f7389]">
            Chain ID is auto-detected from transaction object changes.
          </p>
        </div>
        ...
        <div className="mt-4 rounded-xl border border-[#d6e1ef] bg-[#f4f8ff] p-3">
          ...
          <p className="mt-1 break-all font-mono text-xs text-[#23405f]">
            {chainId || "No chain selected yet."}
          </p>
        </div>
      </section>
      ...
      <section className="surface p-5 md:p-6">
        ...
        <p className="section-subtitle mt-1">
          Move eBL title to the next party and preserve an auditable transfer
          record.
        </p>
        ...
        <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setEndorsementType(Number(e.target.value))}
            >
        ...
        <div className="mt-4">
          ...
          <input
            className="field-input"
            placeholder="e.g. Endorsed to financing bank"
            value={note}
      ...
      {lastTx && (
        <div className="rounded-xl border border-[#c9dff8] bg-[#eef6ff] p-3">
          <p className="text-xs font-semibold uppercase tracking-wide text-[#3a6289]">
            Last transaction
          </p>
          <a
            ...
            className="mt-1 inline-block font-mono text-xs text-[#0e4fbf] underline"
          >
            {lastTx}
          </a>
        </div>
