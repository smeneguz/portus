...
import { EXPLORER_URL, INTEROP_PLATFORM_LABELS, NETWORK, PACKAGE_ID, explorerTxUrl } from '../config/constants';
import { clearTxHistory, getTxHistory, type TxHistoryEntry } from '../utils/txHistory';

type HistoryArea = 'carrier' | 'transfer' | 'surrender' | 'interop' | 'other';
type SourceFilter = 'all' | 'app' | 'chain';
...
type EventQueryConfig = {
  ...
  buildDetails: (parsed: any) => { reference: string; details: string; referenceLabel?: string; timestamp?: number };
};
...
function compactDetails(value: string): string {
  if (!value) return '—';
  const compactAddr = value.replace(/0x[a-fA-F0-9]{20,}/g, (addr) => shortAddr(addr));
  return compactAddr.length > 92 ? `${compactAddr.slice(0, 89)}...` : compactAddr;
}
...
function inferLocalMeta(entry: TxHistoryEntry): { referenceLabel: string; details: string } {
  ...
  if (entry.action === 'Register Carrier') {
    return { referenceLabel: 'CarrierCap ID', details: 'Carrier profile and capability created' };
  }
  if (entry.action === 'Mint CarrierCap') {
    return { referenceLabel: 'CarrierCap ID', details: 'Carrier capability minted' };
  }
  if (entry.action === 'Issue eBL') {
    return { referenceLabel: 'BL Number', details: 'Electronic Bill of Lading issued' };
  }
  if (entry.action === 'Create Endorsement Chain') {
    return { referenceLabel: 'Chain ID', details: 'Chain of title initialized' };
  }
  if (entry.action === 'Endorse & Transfer') {
    return { referenceLabel: 'eBL ID', details: 'Title transferred to next party' };
  }
  ...
  if (entry.action.includes('Interop')) {
    return { referenceLabel: 'Control Object ID', details: 'Interop settlement action' };
  }
  return { referenceLabel: 'Reference', details: 'Wallet-signed transaction' };
}

function fromLocal(entry: TxHistoryEntry): UnifiedHistoryRow {
...
export default function History() {
  ...
  const [localRows, setLocalRows] = useState<UnifiedHistoryRow[]>([]);
  const [chainRows, setChainRows] = useState<UnifiedHistoryRow[]>([]);
    ...
    try {
      ...
      responses.forEach((response, i) => {
        ...
        (response.data as any[]).forEach((ev) => {
          ...
          const eventTimestamp = normalizeTimestamp(built.timestamp ?? ev.timestampMs);
          chain.push({
            source: 'chain',
            area: cfg.area,
            action: cfg.label,
      ...
      console.error(err);
      setError('Failed to load on-chain event history. Check network/package ID.');
    }
  ...
  const enrichedRows = useMemo(() => {
    const chainByDigest = new Map<string, UnifiedHistoryRow>();
    allRows.forEach((row) => {
      if (row.source === 'chain' && row.digest && !chainByDigest.has(row.digest)) {
  ...
  const actionOptions = useMemo(
    () => Array.from(new Set(enrichedRows.map((row) => row.action))).sort((a, b) => a.localeCompare(b)),
    [enrichedRows],
  );
  ...
  }, [enrichedRows, search, sourceFilter, areaFilter, actionFilter, timeFilter]);
  ...
  const appCount = rows.filter((row) => row.source === 'app').length;
  const chainCount = rows.filter((row) => row.source === 'chain').length;
    ...
    <div className="space-y-6">
      <section className="surface p-5 md:p-6">
        ...
        <p className="section-subtitle mt-1">
          Unified timeline from app-signed transactions and package events on-chain.
        </p>
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-5">
          ...
          />
          <select className="field-input" value={sourceFilter} onChange={(e) => setSourceFilter(e.target.value as SourceFilter)}>
            <option value="all">All sources</option>
            <option value="app">App only</option>
            <option value="chain">Chain only</option>
          </select>
          <select className="field-input" value={areaFilter} onChange={(e) => setAreaFilter(e.target.value as AreaFilter)}>
          ...
          <select className="field-input" value={actionFilter} onChange={(e) => setActionFilter(e.target.value)}>
          ...
          <select className="field-input" value={timeFilter} onChange={(e) => setTimeFilter(e.target.value as TimeFilter)}>
        ...
        <div className="mt-3 flex flex-wrap gap-3">
          <button onClick={() => void loadHistory()} disabled={loading} className="btn-main">
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3">
          <div className="rounded-xl border border-[#d7e2ef] bg-white p-3">
            <p className="text-xs uppercase tracking-wide text-[#60758c]">Showing</p>
            <p className="mt-1 text-xl font-bold text-[#173a5a]">
              {rows.length} / {allRows.length}
            </p>
          </div>
          <div className="rounded-xl border border-[#d7e2ef] bg-white p-3">
            <p className="text-xs uppercase tracking-wide text-[#60758c]">Local / Chain</p>
            <p className="mt-1 text-xl font-bold text-[#173a5a]">{appCount} / {chainCount}</p>
          </div>
          <div className="rounded-xl border border-[#d7e2ef] bg-white p-3">
            <p className="text-xs uppercase tracking-wide text-[#60758c]">Latest activity</p>
            <p className="mt-1 text-sm font-semibold text-[#173a5a]">{latestTimestamp ? new Date(latestTimestamp).toLocaleString() : '—'}</p>
          </div>
        </div>
      </section>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full border-collapse">
            <thead className="bg-[#f3f8ff]">
              <tr>
                <th className="border-b border-[#d7e2ef] px-3 py-3 text-left text-xs font-semibold uppercase tracking-wide text-[#5f7389]">Time</th>
                <th className="border-b border-[#d7e2ef] px-3 py-3 text-left text-xs font-semibold uppercase tracking-wide text-[#5f7389]">Source</th>
                <th className="border-b border-[#d7e2ef] px-3 py-3 text-left text-xs font-semibold uppercase tracking-wide text-[#5f7389]">Area</th>
                <th className="border-b border-[#d7e2ef] px-3 py-3 text-left text-xs font-semibold uppercase tracking-wide text-[#5f7389]">Action</th>
                <th className="border-b border-[#d7e2ef] px-3 py-3 text-left text-xs font-semibold uppercase tracking-wide text-[#5f7389]">Reference</th>
                <th className="border-b border-[#d7e2ef] px-3 py-3 text-left text-xs font-semibold uppercase tracking-wide text-[#5f7389]">Details</th>
                <th className="border-b border-[#d7e2ef] px-3 py-3 text-left text-xs font-semibold uppercase tracking-wide text-[#5f7389]">Tx</th>
              </tr>
            </thead>
            <tbody>
              {rows.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-3 py-8 text-center text-sm text-[#5f7389]">
                    No history yet. Execute a transaction, then click refresh.
                  </td>
                </tr>
              )}
              {rows.map((row, i) => (
                <tr key={`${row.digest}-${i}`} className="odd:bg-white even:bg-[#fbfdff]">
                  ...
                  <td className="border-b border-[#ecf2f9] px-3 py-3 text-xs">
                    <span className={`rounded-full px-2 py-1 text-[10px] font-semibold uppercase tracking-wide ${row.source === 'chain' ? 'bg-[#e8f4ef] text-[#0e6a47]' : 'bg-[#e7efff] text-[#1f4d87]'}`}>
                      {row.source}
                    </span>
                  </td>
                  <td className="border-b border-[#ecf2f9] px-3 py-3 text-xs">
                    <span className={`rounded-full px-2 py-1 text-[10px] font-semibold uppercase tracking-wide ${areaClasses(row.area)}`}>
                      {areaLabel(row.area)}
                    </span>
                  </td>
                  <td className="border-b border-[#ecf2f9] px-3 py-3 text-xs font-semibold text-[#173a5a]">{row.action}</td>
                  <td className="border-b border-[#ecf2f9] px-3 py-3 text-xs text-[#2e4d70]">
                    {row.reference ? (
                      <div>
                        <p className="text-[10px] uppercase tracking-wide text-[#60758c]">{row.referenceLabel || 'Reference'}</p>
                        {isCopyableReference(row.referenceLabel, row.reference) ? (
                          <button
                            type="button"
                            onClick={() => void copyText(`ref-${row.digest}-${i}`, row.reference)}
                          ...
                          <p className="mt-1 font-mono text-[11px] text-[#2e4d70]" title={row.reference}>
                            {truncateMiddle(row.reference, 12, 10)}
                          </p>
                        )}
                        {copiedKey === `ref-${row.digest}-${i}` && (
                          <p className="mt-1 text-[10px] font-semibold uppercase tracking-wide text-[#0e6a47]">Copied</p>
                        )}
                      </div>
                    ) : (
                      '—'
                  ...
                  <td className="border-b border-[#ecf2f9] px-3 py-3 text-xs text-[#4f657d]">
                    <span title={row.details}>{compactDetails(row.details)}</span>
                  </td>
                  <td className="border-b border-[#ecf2f9] px-3 py-3 text-xs">
                    {row.digest ? (
                      <a
